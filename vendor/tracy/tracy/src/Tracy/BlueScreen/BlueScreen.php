...
class BlueScreen
{
	...
	public function render(\Throwable $exception): void
	{
		...
		$this->renderTemplate($exception, __DIR__ . '<?= BASE_URL ?>/assets/page.phtml');
	}
	...
	{
		$defer->addSetup('Tracy.BlueScreen.loadAjax', Helpers::capture(fn() => $this->renderTemplate($exception, __DIR__ . '<?= BASE_URL ?>/assets/content.phtml')));
	}
	...
	public function renderToFile(\Throwable $exception, string $file): bool
	{
		if ($handle = @fopen($file, 'x')) {
			...
			$this->renderTemplate($exception, __DIR__ . '<?= BASE_URL ?>/assets/page.phtml', toScreen: false);
			ob_end_flush();
			ob_end_clean();
			fclose($handle);
			return true;
