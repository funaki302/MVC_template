...
class Dumper
{
	...
	public static function renderAssets(): void
	{
		...
		$s = file_get_contents(__DIR__ . '/..<?= BASE_URL ?>/assets/toggle.css')
			. file_get_contents(__DIR__ . '<?= BASE_URL ?>/assets/dumper-light.css')
			. file_get_contents(__DIR__ . '<?= BASE_URL ?>/assets/dumper-dark.css');
		echo "<style{$nonceAttr}>", str_replace('</', '<\/', Helpers::minifyCss($s)), "</style>\n";

		if (!Debugger::isEnabled()) {
			$s = '(function(){' . file_get_contents(__DIR__ . '/..<?= BASE_URL ?>/assets/toggle.js') . '})();'
				. '(function(){' . file_get_contents(__DIR__ . '/..<?= BASE_URL ?>/assets/helpers.js') . '})();'
				. '(function(){' . file_get_contents(__DIR__ . '/../Dumper<?= BASE_URL ?>/assets/dumper.js') . '})();';
